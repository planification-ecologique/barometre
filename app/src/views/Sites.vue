      ...
      <div class="fr-grid-row">
        <div class="fr-col-3">
          <!-- <button id="resetFilters"
            class="fr-btn fr-btn--sm fr-icon-close-circle-line fr-btn--icon-right fr-btn--tertiary fr-mb-3w fr-mt-0"
            @click="resetFilter()">
            Réinitialiser
          </button> -->
        </div>
        <div class="fr-col-4">
          <!-- <select class="fr-select" id="select-them" name="select-them" @change="changeDimension($event)">
            ...
            <option v-for="t in listDimension" :value="t['value']" :key="t['value']" :disabled="t['disabled']">
              {{ t['label'] }}</option>
          </select> -->
        </div>
        ...
        <div class="fr-col-2">
          <!-- <select class="fr-select" id="select-them" name="select-them" @change="changeColoriser($event)">
            ...
            <option v-for="t in listColoriser" :value="t['value']" :key="t['value']" :disabled="t['disabled']">
              {{ t['label'] }}</option>
          </select> -->
        </div>
      </div>
