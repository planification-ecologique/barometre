<template>
    <div class="fr-card fr-card--no-icon fr-card--shadow adjust-height">
        <div class="titleBox">
            <h6 class="cardTitle">{{ dataObj && dataObj.title }}
...
export default {
  ...
  props: {
    ...
    displayChart: {
      type: Boolean, // Assurez-vous que le type est correct
      default: true // Utilisez la valeur par défaut de la prop
    }
  },
    data() {
    return {
      tableData: [] // Définir et initialiser tableData ici
    };
  },
  methods: {
    ...
    toggleChart(type) {
      if (type === 'graphique') {
        this.$emit('update:displayChart', true); // Émettre l'événement pour mettre à jour displayChart
      } else if (type === 'table') {
        this.$emit('update:displayChart', false); // Émettre l'événement pour mettre à jour displayChart
      }
    }
