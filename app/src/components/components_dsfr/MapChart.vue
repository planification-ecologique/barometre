...
import LeftCol from '@/components/components_dsfr/LeftCol'
...
export default {
  ...
  props: {
    ...
    color: {
      type: String,
      default: 'green-bourgeon' //'green-archipel'
    }
  },
  methods: {
    createChart () {      
      ...
      const min_values = Math.min.apply(null, values)
      const max_values = Math.max.apply(null, values)
      min_values == 0 ? this.scaleMin = 0 : this.scaleMin = Math.log10(min_values)
      max_values == 0 ? this.scaleMax = 0 : this.scaleMax = Math.log10(max_values)
      ...
      for (const key in self.dataParse) {
        ...
        if (self.zoomDep === undefined) {
          var color_part = "rgb(238, 238, 238)"
          if (self.dataParse[key] > 0) color_part = x(Math.log10(self.dataParse[key]))
          
          elCol.length !== 0 && elCol[0].setAttribute('fill', color_part)
          self.FranceProps.displayDep[className] = ''
        } else {
          const polygon = document.querySelector('.' + className).getBBox()
          if (self.zoomDep === key) {
            var color_part = "rgb(252, 248, 251)"
            if (self.dataParse[key] > 0) color_part = x(Math.log10(self.dataParse[key]))
            ...
            elCol.length !== 0 && elCol[0].setAttribute('fill', 'rgba(255, 255, , 0)')
      ...
      this.leftColProps.min = min_values //this.scaleMin
      this.leftColProps.max = max_values //this.scaleMax
